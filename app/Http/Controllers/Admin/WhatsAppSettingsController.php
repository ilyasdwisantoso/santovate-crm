...
class WhatsAppSettingsController extends Controller
{
    public function edit(Request $request): Response
    {
        ...
        $tokenReadable = true;
        try {
            $hasToken = filled($channel->access_token);
        } catch (\Throwable $e) {
            report($e);
            $tokenReadable = false;
        }
        ...
        return Inertia::render('Settings/WhatsApp', [
            'channel'=>$channel->only(['provider','phone_number_id','waba_id','is_active','verified_at']),
            'hasToken'=>$hasToken,
            'tokenReadable'=>$tokenReadable,
            'webhookUrl'=>route('api.whatsapp.receive'),
        ]);
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        $channel->phone_number_id = $data['phone_number_id'] ?? null;
        $channel->waba_id = $data['waba_id'] ?? null;
