...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        if ($user) {
            ...
            $org = $user->organization?->load('businessConfiguration');
            if ($org) {
                $configuration = $org->businessConfiguration;
                $organization = [
                    'id'=>$org->id,
                    'name'=>$org->name,
                    'slug'=>$org->slug,
                ...
                if ($active) {
                    $subscription = [
                        ...
                        'plan'=>$active->plan?->only(['id','key','name','user_limit','prospect_limit','features']),
                    ];
                }
            }
        }
